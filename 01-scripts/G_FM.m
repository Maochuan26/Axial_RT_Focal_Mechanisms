...
path    = cfg.dataDir;
path_FM = cfg.hashDir;  % HASH binary + config (not moved — compiled executable)
fields={'AS1','AS2','CC1','EC1','EC2','EC3','ID1'};


%% input and output
%load('/Users/mczhang/Documents/GitHub/FM/02-data/E_Cl_toptenW.mat');
load(fullfile(cfg.dataDir, 'F_Cl.mat'));
outputname = fullfile(path, 'G_FM.mat');
...
for i=1:length(unique([Po.Cluster]))
    ...
    for j=1:length(ind_Cl)
        ...
        for k=1:length(fields) % Noise, Pamp, Samp;
            if eval(strcat('Po(ind_Cl(j)).Po_',fields{k},'(1)>0')) %&& eval(strcat('~isempty(Po(ind_Cl(j)).R',fields{k},')')) %if we don't picks, how can we know the polarity
                str_P=['U'];
            elseif eval(strcat('Po(ind_Cl(j)).Po_',fields{k},'(1)<0'))% && eval(strcat('~isempty(Po(ind_Cl(j)).R',fields{k},')'))
                str_P=['D'];
            else
                continue;
            end
...
% path_FM already set from cfg.hashDir above
